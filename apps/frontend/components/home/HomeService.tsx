...
export default function HomeService(data: ServiceSection): ReactElement {
  ...
  function setItem(index: number) {
    if (serviceRef.current) {
      ...
      setCurrent(index)
      window.scrollTo({
        top: serviceRef.current.scrollHeight + (index + 1) * windowHeight,
        behavior: 'auto',
      })
      const checkIfScrollToIsFinished = setInterval(() => {
        if (serviceRef.current.scrollHeight + (index + 1) * windowHeight === window.scrollY) {
          // do something
          setIsScroll(false)
    ...
    >
      <div className="sticky top-[12%] bottom-[7%]  h-[91vh] block flex-col justify-center items-center overflow-hidden">
        <div className="relative container w-full h-full">
          <div className="text-center text-[80px] leading-[96px] font-[275]">{data.headline}</div>

          <div className="absolute left-[1.5rem] top-[40%] z-50 h-full py-10">
          ...
          >
            ...
            >
              <div className="z-0 overflow-hidden h-full w-full">
                {data.items.map((item, index) => (
                  <div className="absolute justify-center items-center w-full" key={item.headline}>
                    <Service
                      isScroll={isScroll}
                      item={item}
                      index={index}
