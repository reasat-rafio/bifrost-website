    ...
    <div className={className}>
      <Listbox value={selectedData} onChange={setSelectedData}>
        <div className="relative">
          <Listbox.Label className="text-sm leading-5 font-bold text-neutral-600 mb-1 inline-block">
            {label}
          </Listbox.Label>
          <div>
            <Listbox.Button className="datasetInputs mr-auto flex">{selectedData}</Listbox.Button>
            <Listbox.Options className="absolute w-full mt-2 overflow-auto max-h-80 border rounded-[6px] border-[#C9FF71]/20 background__dark  scrollbar-thumb-honeySuckle scrollbar-thin scrollbar-track-gray-100 py-2">
              {data.map(({ name, _id }) => (
                <Listbox.Option
