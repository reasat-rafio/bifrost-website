...
export const useScrollspy = ({
  ...
  const isElementInViewport = useCallback(
    (element: Element) => {
      ...
      return [elementScrollTop < innerScrollBottom, elementScrollBottom > innerScrollTop].every(
        (v) => v,
      )
    },
    [rootEl, offset],
  ...
  const [elementsStatusInViewport, updateElementsStatusInViewport] = useState<boolean[]>([])
