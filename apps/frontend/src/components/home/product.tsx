    ...
    <section className="container z-10 | xl:py-40 lg:py-20 py-16 | text-white">
      <div className="relative grid lg:grid-cols-2 grid-cols-1 | space-y-5 lg:space-y-0 xl:space-x-12 lg:space-x-8 ">
        <motion.div
          ...
          className="grid grid-cols-2 grid-rows-2 | xl:gap-4 gap-3"
        >
          {images.map((image) => (
            <motion.figure
              key={image._key}
              variants={SlideUpChild()}
              className="lg:max-h-[262px] w-full rounded-[8px] aspect-square | overflow-hidden"
            >
              <SanityImg
        ...
        <motion.section
          ...
          viewport={{ once: true }}
          transition={{ type: 'tween', duration: 0.3 * images.length, ease: 'easeInOut' }}
          className="flex flex-col | 2xl:p-14 lg:p-8 p-5 2xl:space-y-6 space-y-3 | border border-[#4e6181]/30 bifrost__transparent__card"
        >
