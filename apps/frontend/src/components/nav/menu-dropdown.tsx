          ...
          >
            <ul className="flex flex-col space-y-8 relative z-20">
              {data?.map(({ _key, description, image, title, ctaButton }) => (
                <li key={_key} className="flex space-x-5 ">
...
const Pointer = () => {
  return (
    <div
      ...
      className="absolute top-0 h-[30px] w-[30px] bg-[#0A0A0E] -translate-y-1/2 -rotate-45 border-gray/60 border-t border-r rounded"
    ></div>
  )
}
