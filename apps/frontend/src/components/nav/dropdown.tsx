    ...
    <AnimatePresence>
      {showNavDropDown && windowWidth < 1024 && (
        <motion.nav
          ...
          className={clsx(
            "| fixed z-30 w-full backdrop-blur-3xl",
            darkBg ? "bg-black/90" : "bg-secondary/5"
          )}
        >
          <ul className="| container flex flex-col items-center justify-center space-y-4 p-5">
            {menu
                ...
                >
                  ...
                  {router.asPath === pageUrl && (
                    <motion.div
                      ...
                      className="primary__gradient absolute left-0 h-[0.2em] w-full"
                    />
                  )}
                </li>
              ))}
